(function ($) {
    ...
    $.fn.gMapType = function (settings)
    {
        return this.each(function () {
            var map_el = $(this);
            map_el.data('map', new GoogleMap(settings, map_el));
            map_el.data('map').initMap();
        });
    };

    function GoogleMap(settings, map_el)
    {
        var settings = $.extend({
    ...
    GoogleMap.prototype = {
        ...
        updateLocation: function (location) 
        {
            ...
            this.geocoder.geocode({'latLng': location}, function (results, status) {
                if (status === google.maps.GeocoderStatus.OK) {
                    $this.settings.country_field.val($this.getAddresComponent('country', results[0], false));
                    $this.settings.city_field.val($this.getAddresComponent('locality', results[0], false));
                    $this.settings.zipcode_field.val($this.getAddresComponent('postal_code', results[0], false));
                    $this.settings.streetname_field.val($this.getAddresComponent('route', results[0], false));
                    $this.settings.streetnumber_field.val($this.getAddresComponent('street_number', results[0], false));
                }
            });
         ...
         */
        getAddresComponent: function (type, geocodeResponse, shortName) 
        {
